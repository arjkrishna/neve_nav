    ...
    for p in worker_log_paths:
        ...
        with p.open("r", errors="replace") as f:
            for line in f:
                ...
                # Perf #4: String pre-check to route to correct regex
                # Avoids expensive backtracking regex attempts on wrong format
                ...
                if "STEP" in line:
                    # Route to correct STEP regex based on string content
                    step_matched = False
                    if "exec_action=" in line:
                        # V3 format (cmd_action + exec_action + mask_reason)
                    ...
                    if not step_matched and "action=" in line:
                        # V2 format (action/cmd_action, term_reason, delta_ins, no exec_action)
                    ...
                    if not step_matched:
                        # OLD format (fallback for existing logs)
                ...
                # EPISODE_END (only reached if line didn't contain STEP or EPISODE_START)
