"""
DualDeviceNav training using BenchAgentSingle (single-process, no multiprocessing).

This version uses make_non_mp() for SOFA simulation to avoid multiprocessing issues.
Use this if you encounter crashes with BenchAgentSynchron or want simpler single-process training.
"""
import os
import logging
import argparse
...
from util.env_dualdevice import BenchEnvDualDevice
from util.agent import BenchAgentSingle
from eve_rl import Runner
from eve_bench import DualDeviceNav


RESULTS_FOLDER = os.getcwd() + "/results/eve_paper/neurovascular/full/mesh_ben_single"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DualDeviceNav training with BenchAgentSingle")
    parser.add_argument(
        "-d",
        "--device",
        type=str,
    ...
    parser.add_argument(
        "-n", "--name", type=str, default="test_single", help="Name of the training run"
    )
    ...
    try:
        ...
        env_train = BenchEnvDualDevice(intervention=intervention, mode="train", visualisation=False)
        
        print("Creating evaluation environment...")
        intervention_eval = DualDeviceNav()
        env_eval = BenchEnvDualDevice(
        ...
        print(f"Final reward: {reward}, Success: {success}")
        print("="*80)
