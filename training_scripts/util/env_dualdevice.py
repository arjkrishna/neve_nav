...
class BenchEnvDualDevice(eve.Env):
    """
    Special BenchEnv for DualDeviceNav that ALWAYS uses single-process mode.
    
    DualDeviceNav with mesh files doesn't work in SOFA multiprocessing mode,
    so we force make_non_mp() regardless of visualization setting.
    """
    def __init__(
        self,
        ...
        # BenchAgentSynchron handles multiprocessing at the agent level
        # Each worker gets its own single-process SOFA scene
        print("[INFO] Using single-process SOFA for DualDeviceNav (agent handles multiprocessing)")
