...
def get_result_checkpoint_config_and_log_path(all_results_folder, name, create_diagnostics=True):
    ...
    mkdir_recursive(config_folder)
    
    # Create diagnostics folder structure
    diagnostics_folder = None
    if create_diagnostics:
        diagnostics_folder = os.path.join(results_folder, "diagnostics")
        mkdir_recursive(diagnostics_folder)
        mkdir_recursive(os.path.join(diagnostics_folder, "csv"))
        mkdir_recursive(os.path.join(diagnostics_folder, "tensorboard"))
        mkdir_recursive(os.path.join(diagnostics_folder, "probes"))
        mkdir_recursive(os.path.join(diagnostics_folder, "plots"))
        mkdir_recursive(os.path.join(diagnostics_folder, "policy_snapshots"))
    
    return main_resultfile, checkpoint_folder, results_folder, log_file, config_folder, diagnostics_folder
