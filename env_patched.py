...
class BenchEnv(eve.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        if self._episode_count > 0 and self._episode_start_time is not None:
            ...
            self._step_logger.info(
                ...
                f"avg_step_time={episode_duration/max(1,self._episode_step_count):.3f}s | wall_time={time.time():.6f} | pid={os.getpid()}"
            )
            sys.stderr.flush()
        ...
        self._step_logger.info(f"EPISODE_START | ep={self._episode_count} | global_steps={self._step_count} | wall_time={time.time():.6f} | pid={os.getpid()}")
        sys.stderr.flush()
    ...
    def step(self, action):
        ...
        log_msg = (
            ...
            f"global={self._step_count} | wall_time={time.time():.6f} | pid={os.getpid()} | cmd_action={action_str} | "
        )
