...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Visualize all verified models sequentially - As stEVE Sees It",
    ...
    print("="*80)
    print("Visualize All Verified Models - As stEVE Sees It")
    print("="*80)
    ...
    for i, model_name in enumerate(models, 1):
        ...
        # Construct path to dualdevicenav_format folder
        model_folder = os.path.join(args.vmr_root, model_name)
        dualdevicenav_folder = os.path.join(model_folder, "dualdevicenav_format")
        
        # Check if processed
        if not os.path.exists(dualdevicenav_folder):
            print(f"[SKIP] Model not processed yet: {model_name}")
            print(f"       Missing: {dualdevicenav_folder}")
            print(f"       Run processing script first!")
            print()
        ...
        # Visualize
        try:
            visualize_as_steve_loads_it(dualdevicenav_folder)
